...
void multMatVetV2(MatRow mat, Vetor v, int m, int n, Vetor res) {
        if (res) {
                ...
                for (i = 0; i < m - m % UNROLL_FACTOR; i += UNROLL_FACTOR) {
                        for (j = 0; j < n; j++) {
                                res[i] += mat[n * i + j] * v[j];
...
void multMatMatV2(MatRow A, MatRow B, int n, MatRow C) {
        ...
        for (int i = 0; i < n; i += BLOCKING_FACTOR) {
                for (int j = 0; j < n; j += BLOCKING_FACTOR) {
                        for (int k = 0; k < n; k += BLOCKING_FACTOR) {
                                for (int ii = i; ii < i + BLOCKING_FACTOR && ii < n; ++ii) {
                                        for (int jj = j; jj < j + BLOCKING_FACTOR && jj < n - (n % 4); jj += 4) {
                                                C[ii * n + jj] = 0.0;
                                                C[ii * n + jj + 1] = 0.0;
                                                C[ii * n + jj + 2] = 0.0;
                                                C[ii * n + jj + 3] = 0.0;
                                                for (int kk = k; kk < k + BLOCKING_FACTOR && kk < n; ++kk) {
